...
class TodoSuite extends FunSuite:
  ...
  def assertTask(modelName: String, actual: Option[Task], expected: Task)(using loc: munit.Location): Unit =
    actual match
      case None =>
  ...
  def assertTaskActive(modelName: String, task: Option[Task])(using loc: munit.Location): Unit =
    task match
      case None =>
