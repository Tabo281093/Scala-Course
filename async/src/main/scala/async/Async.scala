...
object Async extends AsyncInterface {
  ...
  def transformSuccess(eventuallyX: Future[Int]): Future[Boolean] = {
    val t = eventuallyX.flatMap(x => Future(x % 2 == 0))
    t
  }
  ...
  def recoverFailure(eventuallyX: Future[Int]): Future[Int] ={
    val t = eventuallyX.recoverWith{x => Future.successful(-1)}
    t
  }
  ...
  def sequenceComputations[A, B](
    ...
    val t = makeAsyncComputation1().flatMap(x => makeAsyncComputation2().map(c => (x, c)))
    t
  ...
  def concurrentComputations[A, B](
    ...
    val t = makeAsyncComputation1().zip(makeAsyncComputation2())
    t
  }
