...
case class WikiResult[A](value: Future[Either[Seq[WikiError], A]]):
  ...
  def map[B](f: A => B)(using ExecutionContext): WikiResult[B] = {
    val t: Future[Either[Seq[WikiError], B]] = value.map {
      case Left(error) => Left(error)
      case Right(exito) => Right(f(exito))

      // x => x.map(x1 => f(x1)) mi primer forma de escribirlo no fue mala pero sirvio
    }
      WikiResult(t)
    }
...
object WikiResult:
  ...
  def traverse[A, B](as: Seq[A])(f: A => WikiResult[B])(using ExecutionContext): WikiResult[Seq[B]] =
    as.map(x => f(x)).foldLeft(WikiResult.successful(Seq.empty[B]))((accumulated, currentElement) => {
