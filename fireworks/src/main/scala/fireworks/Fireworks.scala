...
object Firework:
  ...
  def next(firework: Firework): Firework =
    firework match
      case w@fireworks.Waiting(_,_,_,_)=> w.next
      case l@fireworks.Launched(_,_,_,_,_)=> l.next
      case e@fireworks.Exploding(_,_) => e.next
      case _@fireworks.Done => Done
...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework = {
    if (countDown > 0) {
      copy(countDown = countDown - 1,Motion.movePoint(position,direction,Settings.propulsionSpeed),direction,numberOfParticles, particlesColor)
      //Launched(countDown-1,Motion.movePoint(position,direction,Settings.propulsionSpeed),direction,numberOfParticles, particlesColor)
    } else {
